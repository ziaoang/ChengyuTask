...
try:
	...
	print("kind(pre/aft) train_or_test(train/test)")
	exit()
...
filename_model = "data/model/%s.model"%kind
filename_res = "data/res/%s.txt"%kind
...
print('Build model...')
model = Sequential()
model.add(Embedding(len(word_indices) + 1, 512, input_length=maxlen))
